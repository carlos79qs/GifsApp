...
describe('useGifs', () => {
	...
	test('should return a list of gifs', async () => {
		...
		//! Importante: el await en los dos lados
		await act(async () => {
	...
	test('should return a list of gifs from cache', async () => {
		...
		// Para poder espiar este metodo, al necesitar dos argumentos lo hemos importado con un nombre diferente "gifActions"
		vi.spyOn(gifActions, 'getGifsByQuery').mockRejectedValue(
			new Error('This is my custom error'),
		);

		// Lógica de esta prueba: Para probar que usa el cache si es un termino repetido,
		//volvemos a llamar con el mismo, al a ver sobrescrito el metodo de llamada a la API para que de error,
		//si lo usara no pasaría el test
		await act(async () => {
			await result.current.handleTermClicked('goku');
		});
	...
	test('should return no more than 8 previous terms', async () => {
		...
		// Sobrescrivimos la llamada para evitar saturar la API
		...
		// Asi nos aseguramos siempre elimina el último elemento del arreglo
